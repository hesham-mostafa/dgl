...
def to_heterogeneous(
    ...
    * Calling :func:`~dgl.to_homogeneous` then calling :func:`~dgl.to_heterogeneous` again
    ...
    ntype2ngrp = {ntype: node_groups[ntid]
...
def bipartite_from_networkx(
    ...
    if u_attrs is not None:
        ...
        for attr in u_attrs:
            g.srcdata[attr] = F.copy_to(
                _batcher(src_attr_dict[attr]), g.device)
    ...
    if v_attrs is not None:
        ...
        for attr in v_attrs:
            g.dstdata[attr] = F.copy_to(
                _batcher(dst_attr_dict[attr]), g.device)
